...
public class EntryJSONConverter {
  ...
  private List<Group> getGroups(schema.Usage usage) {
    ...
    for (int i = 0; i < usage.groupsLength(); i++) {
      ...
      if (g.definitionsLength() > 0)
        group.setDefinitions(this.getDefinitions(new DefinitionsObject(g)));
  ...
  private List<Usage> getUsages(schema.Etymology etymology) {
    ...
    for (int i = 0; i < etymology.usagesLength(); i++) {
      ...
      String pos = POS.name(u.pos());

      if (pos.length() > 0)
        usage.setPOS(pos);
      if (u.groupsLength() > 0)
        usage.setGroups(this.getGroups(u));
      if (u.definitionsLength() > 0)
        usage.setDefinitions(this.getDefinitions(new DefinitionsObject(u)));
  ...
  private List<Etymology> getEtymologies(schema.Entry entry) {
    ...
    for (int i = 0; i < entry.etymologiesLength(); i++) {
      ...
      if (description.length() > 0)
        etymology.setDescription(description);
      if (ety.usagesLength() > 0)
        etymology.setUsages(this.getUsages(ety));
